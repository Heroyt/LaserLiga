...
final readonly class PreparePhotoArchiveJob implements Job
{
	...
	public function getName(): string {
		return 'Prepare photo archives';
	}

	public function run(JobLock $lock): void {
		$logger = new Logger(LOG_DIR, 'cron_photo_archive');

		$codes = DB::select(Photo::TABLE, 'game_code')
			->where('game_code IS NOT NULL AND game_code <> \'\' AND in_archive = false')
			->where(
				'game_code NOT IN %sql',
				DB::select(PhotoArchive::TABLE, 'game_code')
