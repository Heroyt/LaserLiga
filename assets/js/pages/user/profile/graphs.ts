...
export default function initGraphsTab(graphsTabBtn: HTMLElement, colors: string[]) {
	...
	import(
		...
		.then(localeModule => {
			...
			const rankOrderChart = new Chart(
				rankOrderCanvas,
				{
					type: "line",
					data: {
						...
						datasets: [
							{
								label: rankOrderCanvas.dataset.label,
								data: [],
							},
						],
					},
			...
			function loadGraphs() {
				...
				axios.get(`/user/${userCode}/stats/gamecounts?limit=${graphsHistoryFilter.value}`)
					.then((response: AxiosResponse<{
						[index: string]: {
							label: string,
							modes: { count: number, id_mode: number, modeName: string }[]
						}
					}>) => {
						loaded++;
						if (!graphsLoaded) {
